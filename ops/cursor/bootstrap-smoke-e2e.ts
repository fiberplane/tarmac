...
const renderPrompt = (issueId: string): string =>
  [
    ...
    "",
    "Run this exact read-only shell checklist from the repository root:",
    "```bash",
    "set -eu",
    "test -f .cursor/environment.json",
    "grep -q 'ops/cursor/bootstrap-env.sh' .cursor/environment.json",
    "test -f ops/cursor/bootstrap-env.sh",
    ...
    "```",
    "",
    "If every command succeeds, respond with exactly this JSON object and no other text:",
