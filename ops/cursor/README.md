...
must stay idempotent because Cursor may reuse or refresh cached environments.
Set `FP_VERSION` or `DRIFT_VERSION` before bootstrap only when the pinned
versions need to change.
